...
static uint32_t timecount=0 , plauseTime=0 , timeLedOn = 0 , timetimer = 0 , timehall = 0, timetimer2=0;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
	if(GPIO_Pin == GPIO_PIN_13)
	{
		if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET)
		{
			...
			HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_RESET);
			timecount = htim11.Instance->CNT;
			while(htim11.Instance->CNT - timecount < plauseTime )
			{
				count++;
			}
			timeLedOn = htim11.Instance->CNT;
			HAL_GPIO_WritePin(LD2_GPIO_Port, LD2_Pin, GPIO_PIN_SET);
		}
		else if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_SET && htim11.Instance->CNT - timecount >= plauseTime)
		{
